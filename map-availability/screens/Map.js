...
function findClosestPolylinePoint(destination) {
...
export default function Map() {
  ...
  function simplifyRoute(route) {
    ...
    for (let i = 1; i < route.length - 1; i++) {
      ...
      // If the change in bearing is significant, keep this point
      if (Math.abs(prevBearing - nextBearing) > 10) {
        simplifiedRoute.push(current);
      }
    }
    simplifiedRoute.push(route[route.length - 1]);
  ...
  const checkRouteTimeout = useCallback(() => {
    if (isRouteActive && routeStartTime) {
      const currentTime = Date.now();
      const elapsedTime = (currentTime - routeStartTime) / 1000 / 60; // Convert to minutes
      if (elapsedTime >= 15) {
        handleExitRoute();
      }
    }
  }, [isRouteActive, routeStartTime, handleExitRoute]);
  const handleExitRoute = useCallback(() => {
    setRoute([]);
    setDestination(null);
    setBearing(0);
    ...
    setSearchQuery('');
    setDestinationCoords(null);
    setRouteStartTime(null); // Reset the route start time
    ...
    if (location && destination) {
      ...
      if (routeCoordinates.length > 1) {
        ...
        if (start && end) {
          ...
          const remainingRoute = routeCoordinates.slice(currentSegment);
          const remainingDistance = calculateTotalDistance(remainingRoute);
          const remainingDistanceInFeet = Math.round(Math.round(remainingDistance * 3.28084) / 5) * 5;
          const updatedEstimatedTime = Math.ceil(remainingDistanceInFeet / 308);
          setEstimatedTime(updatedEstimatedTime);
  ...
  const animateCamera = useCallback((targetLocation, targetBearing) => {
    if (nearestPoint) {
      mapRef.current?.animateCamera({
        ...
        heading: isRouteActive ? targetBearing : 0,
        pitch: 0,
        zoom: 18,
        altitude: 1000,
      }, { duration: 3000 });
    }
  }, [nearestPoint, isRouteActive]);
