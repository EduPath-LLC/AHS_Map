...
export default function Map({userId}) {
  ...
  const handleSearch = useCallback(async () => {
    if (nearestPolylinePoint && searchQuery) {
      ...
      const isDuplicate = searchHistory.some(search => search.query === newSearch.query);
  ...
  }, [routeFromHome.params?.roomNumber, handleSearch]);
  ...
  const removeItem = async (queryToRemove) => {
    try {
          ...
          {showHistory && (
            <ScrollView style={styles.historyContainer}>
              {searchHistory.map((item, index) => (
                <View style={styles.historyItemContainer} key={index}>
                  <TouchableOpacity 
                    style={styles.historyItemTextContainer}
                    onPress={() => { setSearchQuery(item.query); setShowHistory(false); }}>
                    <Text style={styles.historyItemText}>{item.query}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => removeItem(item.query)}>
